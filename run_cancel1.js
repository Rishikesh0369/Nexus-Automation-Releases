...
(async () => {
    ...
    try {
        ...
        await page.locator('a').filter({ hasText: /My Application/i }).first().hover();
        await page.waitForTimeout(1000);

        console.log('🖱️ "LPG One" पर क्लिक कर रहे हैं (नया टैब खुलेगा)...');
        const [newPage] = await Promise.all([
            context.waitForEvent('page'),
            page.locator('a').filter({ hasText: /LPG One|LPG 1|LPG Next/i }).first().click() 
        ]);

        await newPage.waitForLoadState('domcontentloaded');
        console.log('✅ नया टैब खुल गया!');

        // 4. Cancel Cash Memo पर क्लिक
        console.log('🖱️ "Cancel Cash Memo" पर क्लिक कर रहे हैं...');
        await newPage.locator('a').filter({ hasText: /Cancel Cash Memo|Cash Memo Cancel/i }).first().click();
        await newPage.waitForLoadState('domcontentloaded');
        ...
        for (let i = 0; i < numbers.length; i++) {
            ...
            try {
                ...
                if (await cancelLink.isVisible()) {
                    ...
                    console.log(`⏭️ ${consumerNo} के लिए कोई कैंसिल लिंक नहीं मिला (शायद मेमो है ही नहीं)।`);
                }
