...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    function updateAgencyBadge(status, agencyName = '', customMessage = '') {
        ...
        if (status === 'verified') {
            ...
            if (!isProcessRunning) {
                ...
                btnStartCancellation.classList.remove('btn-subscription-expired', 'btn-danger-pulsing');
            ...
            if (serverMessage.toLowerCase().includes('expired')) {
                ...
                // If license failed cloud verification or offline, allow local runner to proceed
                btnStartCancellation.disabled = false;
                btnStartCancellation.classList.remove('btn-subscription-expired', 'btn-danger-pulsing');
                btnStartCancellation.classList.add('btn-primary');
                btnStartText.textContent = 'Start Auto-Cancellation';
    ...
    function processManifestResponse(response) {
        ...
        if (response.success === false || response.error === 'INVALID_LICENSE' || response.status === 403) {
            ...
            if (response.status === 403 || response.isExpired || response.error === 'SUBSCRIPTION_EXPIRED' || serverMessage.toLowerCase().includes('expired')) {
                const formattedExp = expDate ? new Date(expDate).toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' }) : '';
            ...
            appendLog(`❌ License Error: ${serverMessage}`, 'error');
            return;
        }
        ...
        try {
            ...
            if (window.electronAPI && window.electronAPI.verifyLicense) {
                try {
                    ...
                    return { success: false, status: 0, message: ipcMessage, expiry: ipcDate, expiresAt: ipcDate };
                }
            }
            return { success: false, status: response?.status || 0, message: serverMessage, expiry: serverDate, expiresAt: serverDate };
        }
    }
        ...
        try {
            ...
            if (!bpclUserId || !licenseKey) {
                ...
                if (res && res.success && res.status !== 400 && res.status !== 403) {
                    ...
                    updateAgencyBadge('invalid', '', serverMessage);
                    if (res?.status === 403 || res?.isExpired || res?.error === 'SUBSCRIPTION_EXPIRED' || serverMessage.toLowerCase().includes('expired')) {
    ...
    btnSaveSettings.addEventListener('click', async () => {
        ...
        try {
            ...
            updateAgencyBadge('invalid', '', serverMessage);
            showNotification(serverMessage, 'error');
            if (error.response?.status === 403 || error.status === 403 || error.error === 'SUBSCRIPTION_EXPIRED' || serverMessage.toLowerCase().includes('expired')) {
                // Top Header Banner displays server message directly
                if (banner) {
                    banner.innerText = serverMessage;
                }
    ...
    if (window.electronAPI) {
        ...
        window.electronAPI.onProcessComplete((stats) => {
            closeStopConfirmModal();
            setButtonState('idle');
            ...
            if (stats && stats.success > 0) {
                sendTelemetry(stats.success, currentAgencyName || (agencyDisplay ? agencyDisplay.textContent.replace(/^[🟢🔴⚪\s]+/, '') : ''));
            }
            if (stats && stats.reportPath) {
        ...
        try {
            ...
            const res = await fetch(`${WORKER_BASE_URL}/api/telemetry`, {
                ...
                body: JSON.stringify({
                    ...
                    agencyName: (agencyName || currentAgencyName || (agencyDisplay ? agencyDisplay.textContent.replace(/^[🟢🔴⚪\s]+/, '') : '') || '').trim()
                })
            }).catch(() => null);
