...
ipcMain.handle('start-cancellation', async (_event, options = {}) => {
    ...
    try {
        ...
        triggerCompletionNotification(stats);
...
function triggerCompletionNotification(stats) {
    try {
        if (Notification.isSupported()) {
            ...
            const notif = new Notification({
                ...
                body: `E-Day End Process Complete! Successfully cancelled: ${successCount}/${totalCount} memos.`
            });
            if (stats?.reportPath) {
                notif.on('click', () => {
                    shell.openPath(stats.reportPath).catch(() => {});
                });
            }
            notif.show();
        }
    } catch (err) {
        safeConsoleLog(`[NOTIF] Desktop notification notice: ${err.message}`);
    }
}
